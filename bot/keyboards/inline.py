...
async def account_markup(user_id: int) -> types.InlineKeyboardMarkup | None:
    ...
    print(customer is True)
    if customer:
        print(customer['activated'] is True)
        if customer['activated']:
            keyboard_markup.inline_keyboard[0].append(types.InlineKeyboardButton(text = "Тривога", callback_data = "alarm"))
        else:
            keyboard_markup.inline_keyboard[0].append(types.InlineKeyboardButton(text = "Отримати доступ до Охорони", callback_data = "get_guard_access"))
    return keyboard_markup
    

def get_guard_access_markup(user_id: int, url: str) -> types.InlineKeyboardMarkup | None:
    keyboard_markup = types.InlineKeyboardMarkup(inline_keyboard = [
            [
                types.InlineKeyboardButton(text = "Оплатити", url = url),
            ]
        ]
    )
    return keyboard_markup
