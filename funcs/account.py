...
@account_router.callback_query(F.data.in_({'edit_fullname', 'edit_city', 'edit_description'}))
...
@account_router.callback_query(AccountEdits.city, F.data.in_(get_cities()))
async def change_user_city_handler(query: types.CallbackQuery, state: FSMContext):
    ...
    if not await Customer.check_user_exists(user_id) and not await Guards.check_user_exists(user_id):
        ...
        await query.message.answer("Ваш кабінет: ", reply_markup = await get_user_account_markup(user_id))
...
@account_router.callback_query(F.data.in_({"accept_city_changing", "cancle_city_changing"}))
...
@account_router.callback_query(F.data.in_({"cancle_account_deleting", "accept_account_deleting"}))
